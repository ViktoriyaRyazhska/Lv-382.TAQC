...
namespace SoftTasks.CodeWars.Level6
{
    class RainFall
    {
        ...
        public static double Mean(string town, string strng)
        {
            ...
            return GetValues(town, strng).Average();
        }

        public static double[] GetValues(string town, string strng)
        {
            double[] values;
            int index = strng.IndexOf(town) + town.Length + 2;
            int indexEnd = strng.IndexOf("\n", index);

            string line;
            string[] lines;
            if (indexEnd < 0) line = strng.Substring(index);
            else line = strng.Substring(index, indexEnd - index);

            lines = line.Split(',');
            values = new double[lines.Length];
            for (int i = 0; i < values.Length; i++)
            {
                values[i] = double.Parse(lines[i].Split(' ')[1]);
            }
            return values;
        }
